...
setup.resolveStandeeCharacters = function () {
  ...
  if (/^3장 · /.test(title)) {
    if (/새라에게 한 고백|새라가 가져온 대답|직접 고른 저녁|하나뿐인 열쇠|새라가 여는 문/.test(title)) {
      return ['sera'];
    }
    if (/유진에게 한 고백|사건 없이 온 대답|아무도 구하지 않은 오후|사 오지 않은 감기약|비상 연락망의 첫째|유진에게 보내는 신호/.test(title)) {
      return ['yujin'];
    }
    if (/채린에게 한 고백|명령 없이 온 대답|예약하지 않은 토요일|혼자 간 국숫집|비어 있지 않은 일정표|채린이 비운 토요일/.test(title)) {
      return ['chaerin'];
    }
    return ['sera', 'yujin', 'chaerin'];
  }
