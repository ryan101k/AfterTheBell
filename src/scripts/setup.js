...
setup.resolveStandeeCharacters = function () {
  ...
  if (/^2장 · /.test(title)) {
    ...
    if (/사건 없는 확인|취소된 저녁|구조받지 않은 사람|먼저 보낸 영화 제목|체온계의 반대편|회복하면 끝나는 사건|묻지 않고 바꾼 시간|이름이 오른 화면|유진의 선|아무 일 없는 보름|취소해도 남은 약속|제복 없는 저녁|오지 않은 안부|먼저 온 답장|비번의 영화|엔딩 크레딧|지키지 않는 밤|야간 순찰의 편의점|캔커피 두 개|현관의 흔적|묻지 않은 이름|먼저 취소한 약속|미룬 저녁|열이 나는 비번|진통제 봉투/.test(title)) {
      return ['yujin'];
    }
    if (/정체를 아는 사람|사교회의 뺨|원작자의 이름|옮겨진 의자|돌려온 자문료|열람권 뒤의 저녁|줄을 선 저녁|답장이 늦은 밤|예약하지 않은 답장|공개되지 않은 이름|다시 명령해 보라는 사람|채린의 선|값이 없는 십 분|비워 둔 토요일|로비의 한채린|값을 매기지 않은 초대|예약 없는 저녁|가격 없는 산책|육교 위의 십 분|직접 고른 선물|영수증 없는 화분|약속된 한 시간|수행원 없는 버스|두 정거장|준비되지 않은 방문|값이 없는 라면|거절된 예약|편의점 우산|천 원짜리 비닐/.test(title)) {
      return ['chaerin'];
    }
    return ['sera'];
  }
  ...
  if (/^3장 · /.test(title)) {
    if (/새라에게서 받은 고백|새라가 가져온 대답|직접 고른 저녁|하나뿐인 열쇠|새라가 여는 문/.test(title)) {
      return ['sera'];
    }
    if (/유진에게서 받은 고백|사건 없이 온 대답|아무도 구하지 않은 오후|사 오지 않은 감기약|비상 연락망의 첫째|유진에게 보내는 신호/.test(title)) {
      return ['yujin'];
    }
    if (/채린에게서 받은 고백|명령 없이 온 대답|예약하지 않은 토요일|혼자 간 국숫집|비어 있지 않은 일정표|채린이 비운 토요일/.test(title)) {
      return ['chaerin'];
    }
    return ['sera', 'yujin', 'chaerin'];
  }
